...
/// Rappresenta un singolo divisore verticale.
/// [time] in formato "yyyy-MM-dd"
/// [colorHex] colore (es. "#ff0000")
/// [leftLabel] testo in alto, lato sinistro della linea
/// [rightLabel] testo in alto, lato destro della linea
...
/// - Divisori verticali personalizzati (time, color, label a sinistra/destra)
///
...
class MultiSeriesLightweightChartWidget extends StatelessWidget {
  ...
  /// Nuova proprietà: lista di divisori verticali
  ...
  /// [verticalDividers] consente di disegnare linee verticali
  ///   con testo a sinistra e a destra della linea.
  MultiSeriesLightweightChartWidget({
    ...
    // Convertiamo l'elenco di serie in JS (stringa JSON)
    final String seriesJsArray = _buildSeriesJsArray(seriesList, simulateIfNoData);

    // Convertiamo l'elenco di divisori verticali in JS (stringa JSON)
    ...
    // Creiamo un blob e relativo URL
  ...
  String _buildHtmlContent(String title, String seriesJsArray, String verticalDividersJsArray) {
    // Tutto l'HTML + CSS + script, con l'aggiunta della sezione
    // per i divisori verticali (verticalDividers).
    return '''
<!DOCTYPE html>
<html lang="it">
<head>
  ...
  <style>
    ...
    #vertical-dividers-container {
      ...
      pointer-events: none;
      overflow: visible;
        z-index: 9999; /* oppure 999 o qualunque valore più alto di quello del chart */
    }
...
/* Etichetta a sinistra:
   Aumentiamo la traslazione X a -110% (o -120% se vuoi più distacco).
   Esempio:
*/
.vertical-divider-label-left {
...
/* Etichetta a destra:
   Aggiungiamo un +10% così il testo parte 10% più a destra della linea.
*/
.vertical-divider-label-right {
...
<body>
  ...
  <script>
    (function(){
      const seriesList = $seriesJsArray; // array di serie
      ...
      window.addEventListener('DOMContentLoaded', () => {
        ...
        // Per semplicità disegniamo TUTTE le serie nel navigator
        seriesList.forEach(s => {
        ...
        window.addEventListener('resize', ()=>{
          ...
          updateNavRectangle();
          // Aggiorniamo posizione linee
          updateVerticalDividers();
        });

        // DIVISORI VERTICALI
        // Creiamo nel DOM, dentro #vertical-dividers-container, un div con classe "vertical-divider-line"
        // e due label (classe vertical-divider-label-left e vertical-divider-label-right) per ogni divider
        ...
        // Per sicurezza, chiamiamo subito
        updateVerticalDividers();
...
''';
  ...
  /// Costruisce l'array JS dei divisori verticali
